...
class TestCreateInstalledAppWhenAppCreated:
    def test_skips_existing_installation(
        self, account: Account, app_model: App, sqlite_session: Session
    ) -> None:
        ...
        handle(app_model, account=account, session=sqlite_session)
    ...
    def test_adds_missing_installation_without_committing(
        ...
        assert installed_app.created_by == account.id
        assert commits == []
