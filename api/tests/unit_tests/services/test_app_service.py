...
def test_get_recent_apps_uses_one_tenant_scoped_projection_query(sqlite_session: Session) -> None:
    ...
    tenant = Tenant(id=tenant_id, name="Recent Apps Workspace")
    ...
    projection_statements = [statement for statement in select_statements if "FROM apps" in statement]
    assert len(projection_statements) == 1
    assert "count(" not in projection_statements[0].lower()
    assert "app_model_configs" not in projection_statements[0].lower()


def test_get_recent_apps_hides_other_creators_from_normal_members(sqlite_session: Session) -> None:
    tenant = Tenant(name="Isolated Apps Workspace")
    account = Account(name="Normal Member", email="normal-member@example.com")
    sqlite_session.add_all([tenant, account])
    sqlite_session.flush()
    sqlite_session.add(
        TenantAccountJoin(
    ...
    own_app = _persist_app(sqlite_session, tenant_id=tenant.id, name="Own App")
    other_app = _persist_app(sqlite_session, tenant_id=tenant.id, name="Other App")
    own_app.created_by = account.id
    other_app.created_by = str(uuid4())
    sqlite_session.commit()
    ...
    assert [app.name for app in recent_apps] == ["Own App"]
