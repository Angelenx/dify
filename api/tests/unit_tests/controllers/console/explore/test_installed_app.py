...
class TestInstalledAppsListApi:
    ...
    def test_get_installed_apps(
        ...
        assert result["installed_apps"][0]["created_by"] == "u1"
    ...
    def test_get_installed_apps_with_app_id_filter(self, app: Flask, current_user: MagicMock, tenant_id: str) -> None:
        ...
        executed_stmt = session.execute.call_args.args[0]
        assert "u1" in executed_stmt.compile().params.values()
...
class TestInstalledAppsCreateApi:
    def test_post_success(
        self, app: Flask, current_user: MagicMock, tenant_id: str, payload_patch: PayloadPatch
    ) -> None:
        api = module.InstalledAppsListApi()
        ...
        with (
            ...
            result = method(
                api,
                module.InstalledAppCreatePayload.model_validate({"app_id": "a1"}),
                tenant_id,
                current_user,
            )
        ...
        assert session.add.call_args.args[0].created_by == current_user.id

    def test_post_recommended_not_found(
        self, app: Flask, current_user: MagicMock, tenant_id: str, payload_patch: PayloadPatch
    ) -> None:
        api = module.InstalledAppsListApi()
        method = unwrap(api.post)
...
def test_sqlite_post_installs_public_recommended_app_and_is_idempotent(
    ...
    for _ in range(2):
        with (
            ...
            assert method(
                api,
                module.InstalledAppCreatePayload.model_validate({"app_id": app_model.id}),
                tenant_id,
                current_user,
...
def test_sqlite_post_enforces_recommendation_and_public_state(
    ...
    with (
        ...
        method(
            api,
            module.InstalledAppCreatePayload.model_validate({"app_id": "missing"}),
            tenant_id,
            current_user,
        )
    ...
    with (
        ...
        method(
            api,
            module.InstalledAppCreatePayload.model_validate({"app_id": private_app.id}),
            tenant_id,
            current_user,
        )
