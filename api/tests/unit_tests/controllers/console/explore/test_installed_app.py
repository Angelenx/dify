...
class TestInstalledAppsListApi:
    ...
    def test_response_schema_preserves_installed_app_domain_types(self) -> None:
        ...
        assert "created_by" in list_schema["$defs"]["InstalledAppResponse"]["required"]
    ...
    def test_get_filters_tenant_publication_mode_and_app_id(
        ...
        other_user = _persist_app(database_session, app_id="other-user")
        ...
        _persist_installed_app(
            ...
            other_user,
            tenant_id=tenant_id,
            installed_app_id="installed-other-user",
            created_by="user-2",
        )
        ...
        assert {item["app"]["id"] for item in result["installed_apps"]} == {"chat", "workflow", "other-user"}
        assert all("created_by" in item for item in result["installed_apps"])
        ...
        assert filtered["installed_apps"][0]["created_by"] == current_user.id
        assert filtered["installed_apps"][0]["editable"] is False

        with (
            app.test_request_context("/?app_id=other-user"),
            _controller_context(database_session, role=TenantAccountRole.NORMAL),
        ):
            filtered = method(api, tenant_id, current_user)
        assert filtered["installed_apps"] == []
...
class TestInstalledAppsCreateApi:
    def test_post_installs_public_recommended_app_once(
        ...
        for _ in range(2):
            with (
                ...
                assert unwrap(module.InstalledAppsListApi().post)(
                    module.InstalledAppsListApi(), request_data, tenant_id, current_user
        ...
        with Session(bind) as observer:
            ...
            assert installed[0].created_by == current_user.id
    ...
    def test_post_enforces_recommendation_and_public_state(
        ...
        with (
            ...
            unwrap(api.post)(api, module.InstalledAppCreatePayload(app_id="missing"), tenant_id, current_user)
        ...
        with (
            ...
            unwrap(api.post)(
                api,
                module.InstalledAppCreatePayload(app_id=private_app.id),
                tenant_id,
                current_user,
            )
